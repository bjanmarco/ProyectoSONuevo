...
int verificarProteccionMemoria(int direccionFisica) {
    ...
    // en modo usuario se permite acceder a toda la zona de usuario
    if (direccionFisica >= INICIO_MEMORIA_USUARIO && direccionFisica < TAMANO_MEMORIA) {
        return 1;
    }
    return 0;
}
...
void ejecutarCpu() {
    ...
    // evitar devolver control a consola con DMA aun en curso
    if (esperarDmaLibre(2000) != 0) {
        imprimirLog("ADVERTENCIA: DMA sigue ocupado al finalizar ejecucion");
    }
...
int faseExecute() {
    ...
    switch (opcode) {
        ...
        case OP_SDMAM:  // 32: direccion de memoria
            ...
            dma.direccionMemoria = direccionFisica; // guarda direccion fisica de RAM
            sprintf(buffer, "DMA: Direccion de memoria establecida a %d (fisica)", direccionFisica);
            imprimirLog(buffer);
            break;
